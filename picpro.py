...
#Is this kosher?  I have no idea...
module_location = os.path.dirname(__file__)
if (module_location == ''):
    module_location = os.curdir + os.sep
else:
    # The following line doesn't appear to be necessary...
    # sys.path = [module_location] + sys.path
    module_location = module_location + os.sep
...
Arguments:
    ...
    for (opt, value) in opts:
        try:
            ...
            print 'Error: Invalid usage of option "' + opt + '".'
            print_usage_and_abort()
    ...
    program_pic(port, pic_type, hex_file_name, program, fuses = fuses,
                ID = ID, icsp_mode = icsp_mode)
...
class Chipinfo_Reader(object):
    ...
    def get_chip(self, name):
        return Chipinfo_Entry(**self.chip_entries[name.lower()])
    ...
    def program_rom(self, data):
        ...
        try:
            for i in xrange(0, (word_count * 2), 32):
                sys.stderr.write("debug ->"+str(i)+":"+str(word_count*2)+"  "+str(data[i:(i+32)])+"\n")
                self.port.write(data[i:(i + 32)])
                self._expect('Y', timeout=50)
            sys.stderr.write("debug ->fin\n")

            self._expect('P', timeout=50)
...
def program_pic(port, pic_type, hex_file_name = '',
    ...
    if (fuses):
        ...
        except Chipinfo_Entry.Fuse_Error:
    ...
    try:
        ...
        if (program):
            ...
            print prot_interface.programmer_protocol()
            print 'Erasing Chip'
            prot_interface._command_start()
            if (not prot_interface.erase_chip()):
                print 'Erasure failed.'

            print "eeprom is blank"
            print prot_interface.eeprom_is_blank()
            print "eeprom is blank"

            print "read eeprom"
            print prot_interface.read_eeprom()
            print "read eeprom"
